...
for(i in 1:boot.reps){
  boot.subs <- generate.sasa.subs(Calphas, b.length = 5, ratio = 1, exponent = 2)
...
dists_1ax8 <- matrix(NA,nrow=reps,ncol=10)
...
for(l in 1:reps){
  ...
  subs <- generate.sasa.subs(Calphas, b.length = 5, ratio = 1, exponent = 2, subs = subs)
...
hominoid_subs <- c(68, 89, 100, 108, 118)
Cdata_sub <- Calphas[is.element(Calphas$pos, hominoid_subs),]
tmp <- sapply(1:5, find.dist, Cdata=Cdata_sub)
dists_hominoid_1ax8 <- tmp[upper.tri(tmp)]



save.image("test2_surface3_hominoid_1ax8.RData")
